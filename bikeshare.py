...
def get_filters():
    ...
    while True:
        city = choice("\nWhich city(ies) will you like to explore, "
                      "New York City, Chicago or Washington? Use commas "
                      "to list the names.\n>", CITY_DATA.keys())
        month = choice("\nSelect from January to June, the month(s) you want to filter the data? "
...
def load_data(city, month, day):
    ...
    print("\nThe program is loading your filters of choice.")
...
def user_stats(df, city):
    ...
    try:
        ...
        print("The youngest person to ride one "
...
def raw_data(df, mark_place):
    ...
    while True:
        for i in range(mark_place, len(df.index)):
            ...
            if choice("Would you like to keep printing raw data?"
                      "\n\n[y]Yes\n[n]No\n\n>") == 'y':
                continue
            else:
                break
